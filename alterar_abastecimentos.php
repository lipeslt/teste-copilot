...
if (isset($conn)) {
    ...
    // header('Location: pagina_de_erro.php'); // Ou redirecionar para uma página de erro
    // exit;
}
...
// Função para processar upload de arquivo (usada por Adicionar e Editar)
function processarUploadNotaFiscal($file_input_name, $upload_sub_dir = "notas_fiscais/") {
    if (isset($_FILES[$file_input_name]) && $_FILES[$file_input_name]['error'] == UPLOAD_ERR_OK) {
        ...
        if (!is_writable($upload_dir)) throw new Exception("Erro de permissão no servidor para upload de nota fiscal (diretório: $upload_dir).");
        ...
        throw new Exception("Erro no upload da nota fiscal. Código: " . $_FILES[$file_input_name]['error']);
    }
    return null; // Nenhum arquivo novo ou erro não crítico
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['registro_id']) && !isset($_POST['action'])) {
    ...
    try {
        ...
        if (!preg_match('/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/', $hora)) throw new Exception("Formato de hora inválido.");
        ...
        if (empty($nome) || empty($data) || empty($hora) || empty($litros) || empty($combustivel_post) || empty($valor)) {
        ...
        $caminho_nota_fiscal_final = $registro_antigo_db['nota_fiscal']; // Começa com o valor antigo

        // Verifica se um novo arquivo de nota fiscal foi enviado para o modal de edição
        $novo_caminho_nota_fiscal = processarUploadNotaFiscal('modal_nota_fiscal_arquivo');

        if ($novo_caminho_nota_fiscal) {
            // Se um novo arquivo foi carregado, apague o antigo (se existir e for um arquivo válido)
        ...
        if ($stmt_update->execute($params_update)) {
            ...
            $conn->rollBack();
            if ($novo_caminho_nota_fiscal && file_exists($novo_caminho_nota_fiscal)) {unlink($novo_caminho_nota_fiscal); error_log("Upload NF (edição) revertido: " . $novo_caminho_nota_fiscal);} // Apaga novo arquivo se DB falhar
    ...
    header("Location: alterar_abastecimentos.php?" . $_SERVER['QUERY_STRING']);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'adicionar') {
    ...
    try {
        ...
        $campos_faltando = [];
        if (empty($nome)) $campos_faltando[] = 'Nome do Motorista'; if (empty($data)) $campos_faltando[] = 'Data';
        if (empty($hora)) $campos_faltando[] = 'Hora'; if (empty($placa)) $campos_faltando[] = 'Placa do Veículo';
        if (empty($veiculo)) $campos_faltando[] = 'Veículo'; if (empty($litros) || $litros <= 0) $campos_faltando[] = 'Litros (>0)';
        if (empty($combustivel_post)) $campos_faltando[] = 'Combustível'; if (empty($valor) || $valor <= 0) $campos_faltando[] = 'Valor (>0)';
        if (!empty($campos_faltando)) throw new Exception("Preencha: " . implode(", ", $campos_faltando));
        $combustiveis_validos = ['Gasolina', 'Etanol', 'Diesel-S10', 'Diesel-S500'];
        if (!in_array($combustivel_post, $combustiveis_validos)) throw new Exception("Tipo de combustível inválido.");
        ...
        foreach ($dados_registro as $campo => $valor_campo_bind) $stmt_insert->bindValue(':' . $campo, $valor_campo_bind);
    ...
    } catch (Exception $e) { // Captura PDOException e Exception genérica
        if (isset($conn) && $conn->inTransaction()) $conn->rollBack();
        ...
        $_SESSION['mensagem'] = "Erro: " . $e->getMessage(); $_SESSION['tipo_mensagem'] = "error";
        error_log("Erro ao adicionar: " . $e->getMessage() . ($e instanceof PDOException ? " | SQLSTATE: " . $e->getCode() : ""));
    }
    header("Location: alterar_abastecimentos.php?" . $_SERVER['QUERY_STRING']);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'excluir') {
    try {
        ...
        if ($stmt_delete->execute()) {
            if ($nota_fiscal_a_excluir && strpos($nota_fiscal_a_excluir, 'uploads/notas_fiscais/') === 0 && file_exists($nota_fiscal_a_excluir)) { // Verifica se é um caminho de upload antes de tentar excluir
            ...
            $conn->rollBack(); $_SESSION['mensagem'] = "Erro ao excluir registro."; $_SESSION['tipo_mensagem'] = "error";
            error_log("Falha no DELETE: " . print_r($stmt_delete->errorInfo(), true));
        }
    } catch (Exception $e) {
        if (isset($conn) && $conn->inTransaction()) $conn->rollBack();
        $_SESSION['mensagem'] = "Erro: " . $e->getMessage(); $_SESSION['tipo_mensagem'] = "error"; error_log("Erro ao excluir: " . $e->getMessage());
    }
    header("Location: alterar_abastecimentos.php?" . $_SERVER['QUERY_STRING']);
    exit;
}

// Configuração de paginação e busca de registros (sem alterações nesta seção para o request atual)
$registros_por_pagina = 20; $pagina_atual = isset($_GET['pagina']) ? (int)$_GET['pagina'] : 1; if ($pagina_atual < 1) $pagina_atual = 1;
$inicio = ($pagina_atual - 1) * $registros_por_pagina; $prefixo_veiculo_filtro = $_GET['prefixo_veiculo'] ?? '';
$data_inicial_filtro = $_GET['data_inicial'] ?? ''; $data_final_filtro = $_GET['data_final'] ?? ''; $combustivel_filtro = $_GET['combustivel'] ?? '';
function buscarRegistros($conn, $prefixo_veiculo, $data_inicial, $data_final, $combustivel, $secretaria_admin, $role, $inicio_offset, $limite) {
    ...
    if (!empty($data_inicial)) { $where_conditions .= ($data_final ? " AND r.data BETWEEN :data_inicial AND :data_final" : " AND r.data >= :data_inicial"); $params[':data_inicial'] = $data_inicial; if ($data_final) $params[':data_final'] = $data_final;
    } elseif (!empty($data_final)) { $where_conditions .= " AND r.data <= :data_final"; $params[':data_final'] = $data_final; }
    ...
    $stmt_count = $conn->prepare($count_query_sql); $stmt_count->execute($params);
    ...
    $stmt_data->bindValue(':inicio_offset', (int)$inicio_offset, PDO::PARAM_INT); $stmt_data->bindValue(':limite', (int)$limite, PDO::PARAM_INT);
    $stmt_data->execute(); return ['registros' => $stmt_data->fetchAll(PDO::FETCH_ASSOC), 'total' => $total_registros];
}
...
$secretaria_admin_sessao = $_SESSION['secretaria'] ?? ''; $secretaria_admin_db = $secretarias_map[$secretaria_admin_sessao] ?? null; $role_sessao = $_SESSION['role'] ?? '';
$resultado = buscarRegistros($conn, $prefixo_veiculo_filtro, $data_inicial_filtro, $data_final_filtro, $combustivel_filtro, $secretaria_admin_db, $role_sessao, $inicio, $registros_por_pagina);
$registros = $resultado['registros']; $total_registros = $resultado['total'];
...
if ($pagina_atual > $total_paginas && $total_paginas > 0) {
    $pagina_atual = $total_paginas; $inicio = ($pagina_atual - 1) * $registros_por_pagina;
    $resultado = buscarRegistros($conn, $prefixo_veiculo_filtro, $data_inicial_filtro, $data_final_filtro, $combustivel_filtro, $secretaria_admin_db, $role_sessao, $inicio, $registros_por_pagina);
    $registros = $resultado['registros']; $total_registros = $resultado['total'];
}
$secretarias_disponiveis = buscarSecretarias(); $combustiveis_disponiveis = buscarCombustiveis();
$data_atual_form = date('Y-m-d'); $hora_atual_form = date('H:i');
try { $stmt_precos = $conn->prepare("SELECT * FROM postos_precos ORDER BY posto_nome, tipo_combustivel"); $stmt_precos->execute(); $precos_combustiveis = $stmt_precos->fetchAll(PDO::FETCH_ASSOC); } catch (PDOException $e) { error_log("Erro ao buscar preços: " . $e->getMessage()); $precos_combustiveis = []; }
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
...
<body class="min-h-screen">
    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <?php if (isset($_SESSION['mensagem'])): ?>
            <div class="<?= $_SESSION['tipo_mensagem'] === 'success' ? 'success-message' : 'error-message' ?>"><?= htmlspecialchars($_SESSION['mensagem']) ?></div>
        ...
        <div class="card p-4 md:p-6">
            ...
            <?php if (count($registros) > 0): ?>
                ...
                <?php endif; ?>
            <?php else: ?>
                <div class="text-center py-8"><i class="fas fa-gas-pump text-4xl text-gray-300 mb-4"></i><h4 class="text-lg font-medium text-gray-700">Nenhum registro encontrado</h4><p class="text-gray-500 mt-2">Não foram encontrados registros para os filtros.</p></div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div id="modalEdicao" class="modal">
        <div class="modal-content">
            ...
            <form id="formEdicao" method="post" enctype="multipart/form-data"> {/* Adicionado enctype */}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <div id="current_nota_fiscal_display_edit" class="text-sm text-gray-600 mb-2 p-2 border border-gray-200 rounded-md bg-gray-50 min-h-[2.5rem] flex items-center"></div>
                        <div class="mt-1 flex flex-col items-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                            <div class="w-full text-center">
                                ...
                                <label for="modal_nota_fiscal_arquivo" class="mt-2 text-sm font-medium text-indigo-600 hover:text-indigo-500 cursor-pointer">
                                    <span>Carregar novo arquivo ou tirar foto</span> (deixe em branco para manter o atual)
                                    <input id="modal_nota_fiscal_arquivo" name="modal_nota_fiscal_arquivo" type="file" class="sr-only" accept="image/*,application/pdf" capture="environment">
                                </label>
                                <p class="text-xs text-gray-500 mt-1">PNG, JPG, GIF, PDF até 5MB</p>
                                <p id="modal_nota_fiscal_filename_display" class="mt-2 text-sm text-gray-600 truncate" title="Nenhum arquivo novo selecionado"></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex flex-col items-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                            <div class="w-full text-center">
                                <svg class="mx-auto h-10 w-10 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true"><path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>
                                <label for="nota_fiscal_arquivo" class="mt-2 text-sm font-medium text-indigo-600 hover:text-indigo-500 cursor-pointer">
                                    <span>Clique para carregar um arquivo ou tirar uma foto</span>
                                    <input id="nota_fiscal_arquivo" name="nota_fiscal_arquivo" type="file" class="sr-only" accept="image/*,application/pdf" capture="environment">
                                </label>
                                <p class="text-xs text-gray-500 mt-1">PNG, JPG, GIF, PDF até 5MB</p>
                                <p id="nota_fiscal_filename_display" class="mt-2 text-sm text-gray-600 truncate" title="Nenhum arquivo selecionado"></p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function abrirModalEdicao(id, nome, data, hora, km, litros, combustivel, posto, valor, notaFiscalAtual) {
            ...
            document.getElementById('modal_km_abastecido').value = km;
            ...
            if (postoEncontrado) postoSelect.value = posto;
            else if (posto) { postoSelect.value = 'outro'; outroPostoContainer.style.display = 'block'; outroPostoInput.value = posto; }
            else postoSelect.value = '';
            
            const currentNotaDisplay = document.getElementById('current_nota_fiscal_display_edit');
            if (notaFiscalAtual && notaFiscalAtual.startsWith('uploads/notas_fiscais/')) {
                 currentNotaDisplay.innerHTML = `Arquivo atual: <a href="${notaFiscalAtual}" target="_blank" class="text-blue-500 hover:underline" title="${notaFiscalAtual.split('/').pop()}">${notaFiscalAtual.split('/').pop().substring(0,25)}${notaFiscalAtual.split('/').pop().length > 25 ? '...' : ''}</a>`;
            } else if (notaFiscalAtual) { // Se for texto, não um path de upload
                 currentNotaDisplay.innerHTML = `Informação atual: <span class="font-semibold">${notaFiscalAtual}</span>`;
            } else {
                 currentNotaDisplay.innerHTML = 'Nenhuma nota fiscal salva.';
            }
            // Resetar o input de novo arquivo e sua exibição
            $('#modal_nota_fiscal_arquivo').val(''); 
            $('#modal_nota_fiscal_filename_display').text('Nenhum arquivo novo selecionado').attr('title', 'Nenhum arquivo novo selecionado');
            
            document.getElementById('modalEdicao').style.display = 'block';
        }
        function fecharModalEdicao() { document.getElementById('modalEdicao').style.display = 'none'; }
        function formatarHora(hora) {
            if (/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(hora)) return hora;
            const p = hora.split(':'); return p.length >= 2 ? `${p[0].padStart(2,'0')}:${p[1].padStart(2,'0')}` : new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
        }
        function abrirModalExclusao(id, veiculo, placa, data) {
            ...
            const d = data.split('-'); document.getElementById('exclusao_data').textContent = 'Data: ' + (d.length === 3 ? `${d[2]}/${d[1]}/${d[0]}` : data);
        ...
        function abrirModalAdicao() {
            ...
            $('#nota_fiscal_arquivo').val(''); // Limpa o input de arquivo explicitamente
        ...
        function buscarVeiculos(termo) {
            if (termo.length < 1) { $('#suggestions').hide(); return; }
            $.ajax({ url: 'buscar_veiuculos_abastecimento.php', type: 'GET', data: { termo: termo, tipo: 'prefixo' }, dataType: 'json',
                success: data => {
                    ...
                    if (data && data.length > 0) data.forEach(v => s.append(`<div class="suggestion-item" onclick="selecionarVeiculo('${v.prefixo.replace(/'/g, "\\'")}')">${v.prefixo} - ${v.placa} (${v.tipo||v.prefixo})</div>`));
                    else s.append('<div class="suggestion-item">Nenhum veículo encontrado</div>');
                    s.show();
                }, error: (xhr) => $('#suggestions').html(`<div class="suggestion-item">Erro: ${xhr.statusText}</div>`).show()
            });
        }
        function selecionarVeiculo(prefixo) { $('#prefixo_veiculo').val(prefixo); $('#suggestions').hide(); }

        function buscarVeiculosPorPrefixo(prefixo) {
            if (prefixo.length < 1) { $('#suggestions_prefixo').hide(); return; }
            $.ajax({ url: 'buscar_veiuculos_abastecimento.php', type: 'GET', data: { termo: prefixo, tipo: 'prefixo_completo' }, dataType: 'json',
                success: data => {
                    ...
                    if (data && data.length > 0) data.forEach(v => {
                        const pfx = (v.prefixo||'').replace(/'/g, "\\'"), plc = (v.placa||'').replace(/'/g, "\\'"), vei = (v.tipo||v.veiculo||v.prefixo||'').replace(/'/g, "\\'"), sec = (v.secretaria||'').replace(/'/g, "\\'");
                        s.append(`<div class="suggestion-item" onclick="selecionarVeiculoCompleto('${pfx}','${plc}','${vei}','${sec}')">${v.prefixo||'N/A'} - ${v.placa||'N/A'} (${v.tipo||v.veiculo||v.prefixo||'N/A'})</div>`);
                    }); else s.append('<div class="suggestion-item">Nenhum veículo encontrado</div>');
                    s.show();
        ...
        function selecionarVeiculoCompleto(prefixo, placa, veiculo, secretaria) {
            ...
            if (placa) $.get('buscar_ultimo_km_abastecimento.php', { placa: placa }, data => { if (data && data.km_abastecido) $('#km_abastecido').val(data.km_abastecido); }, 'json').fail(() => console.log("Erro ao buscar KM."));
        }

        function buscarUsuariosGenerico(termo, containerID, nomeID, idFieldID, secretariaID = null) {
            if (termo.length < 2) { $(`#${containerID}`).hide(); return; }
            $.ajax({ url: 'buscar_usuario.php', type: 'GET', data: { termo: termo }, dataType: 'json',
                success: data => {
                    const s = $(`#${containerID}`).empty().hide();
                    if (data && data.length > 0) data.forEach(u => {
                    ...
                    }); else s.append('<div class="suggestion-item">Nenhum usuário encontrado</div>');
                    s.show();
        ...
        function selecionarUsuarioSimples(nome, userId, nomeID, idFieldID, containerID) { $(`#${nomeID}`).val(nome); $(`#${idFieldID}`).val(userId); $(`#${containerID}`).hide(); }
        ...
        function handlePostoChange(selectElement, outroContainerId, outroInputId, callbackCalc) {
            const outroContainer = document.getElementById(outroContainerId); const outroInput = document.getElementById(outroInputId);
            if (selectElement.value === 'outro') { outroContainer.style.display = 'block'; outroInput.required = true; } 
            else { outroContainer.style.display = 'none'; outroInput.required = false; outroInput.value = ''; }
        ...
        function calcularValorComBaseEmPreco(litrosVal, postoVal, combustivelVal, inputValor) {
            let combustivelParaPreco = combustivelVal; if (combustivelVal === 'Diesel-S500') combustivelParaPreco = 'Diesel';
            if (litrosVal > 0 && postoVal && combustivelVal && postoVal !== 'outro') {
                ...
                if (preco > 0) {
                    inputValor.value = (litrosVal * preco).toFixed(2).replace('.', ',');
                     if (inputValor.id === 'modal_valor') inputValor.value = (litrosVal * preco).toFixed(2);
                }
            }
        }
        function calcularValorAbastecimento() {
            const litros = parseFloat(document.getElementById('litros').value.replace(',', '.')) || 0;
            const posto = document.getElementById('posto').value; const combustivel = document.getElementById('tipo_combustivel').value;
        ...
        function calcularValorAbastecimentoModal() {
            ...
            const posto = document.getElementById('modal_posto_gasolina').value; const combustivel = document.getElementById('modal_combustivel').value;
            calcularValorComBaseEmPreco(litros, posto, combustivel, document.getElementById('modal_valor'));
        }
        
        // Handler para o input de arquivo do modal de Adicionar
        $('#nota_fiscal_arquivo').on('change', function() {
            const filename = $(this).val().split('\\').pop();
            const displayElement = $('#nota_fiscal_filename_display');
            if (filename) {
                displayElement.text('Arquivo selecionado: ' + filename).attr('title', 'Arquivo selecionado: ' + filename);
            } else {
                displayElement.text('Nenhum arquivo selecionado').attr('title', 'Nenhum arquivo selecionado');
            }
        });

        // Handler para o input de arquivo do modal de Editar
        $('#modal_nota_fiscal_arquivo').on('change', function() {
            const filename = $(this).val().split('\\').pop();
            const displayElement = $('#modal_nota_fiscal_filename_display');
            if (filename) {
                displayElement.text('Novo arquivo: ' + filename).attr('title', 'Novo arquivo: ' + filename);
            } else {
                displayElement.text('Nenhum arquivo novo selecionado').attr('title', 'Nenhum arquivo novo selecionado');
            }
        });

        $(document).click(function(event) { if (!$(event.target).closest('.suggestions-container').length) $('.suggestions-list').hide(); });
        $(document).ready(function() { console.log("Página de gerenciamento de abastecimentos carregada."); });
    </script>
</body>
</html>