...
$sql_count = str_replace("SELECT h.*, DATE_FORMAT(h.data_alteracao, '%d/%m/%Y %H:%i:%s') as data_formatada", "SELECT COUNT(*) as total", $sql_base);
...
$sql_usuarios = "SELECT DISTINCT admin_name FROM historico_alteracoes ORDER BY admin_name";
$stmt_usuarios = $conn->prepare($sql_usuarios);
$stmt_usuarios->execute();
$usuarios = $stmt_usuarios->fetchAll(PDO::FETCH_COLUMN);

// Obter lista de campos alterados para o filtro
$sql_campos = "SELECT DISTINCT campo_alterado FROM historico_alteracoes ORDER BY campo_alterado";
    ...
    try {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            if (is_array($data)) {
                ...
                if (isset($data['id']) || isset($data['nome']) || isset($data['veiculo_id'])) {
                    return formatarRegistroVeiculo($data);
                } else if (isset($data['antes']) && isset($data['depois'])) {
                    // Formato especial para comparação antes/depois
                    $html = '<div class="grid grid-cols-1 gap-2">';
                    $html .= '<div><span class="font-semibold text-gray-700">Antes:</span>' . formatarRegistroVeiculo($data['antes']) . '</div>';
                    $html .= '<div><span class="font-semibold text-gray-700">Depois:</span>' . formatarRegistroVeiculo($data['depois']) . '</div>';
                    $html .= '</div>';
                    return $html;
                } else {
...
// Função para formatar registros de veículos de forma mais bonita
function formatarRegistroVeiculo($data) {
    if (!is_array($data)) {
    ...
    $grupos = [
        ...
        'Veículo' => ['veiculo_id', 'placa', 'nome_veiculo'],
        'Quilometragem' => ['km_inicial', 'km_final'],
        'Itinerário' => ['destino', 'ponto_parada'],
        'Data/Hora' => ['data', 'hora', 'hora_final']
    ];
    ...
    $rotulos = [
        ...
        'veiculo_id' => 'Código do Veículo',
        'placa' => 'Placa',
        'nome_veiculo' => 'Modelo',
        'km_inicial' => 'KM Inicial',
        'km_final' => 'KM Final',
        'destino' => 'Destino',
        'ponto_parada' => 'Ponto de Parada',
        'data' => 'Data',
        'hora' => 'Hora Inicial',
    ...
    if (isset($data['km_inicial']) && is_numeric($data['km_inicial'])) {
        $data['km_inicial'] = number_format($data['km_inicial'], 0, ',', '.');
    }
    
    if (isset($data['km_final']) && is_numeric($data['km_final'])) {
        $data['km_final'] = number_format($data['km_final'], 0, ',', '.');
    }
    ...
    foreach ($grupos as $nomeGrupo => $campos) {
        ...
        foreach ($campos as $campo) {
            if (isset($data[$campo]) && $data[$campo] !== '') {
                ...
                $conteudoGrupo .= '<span class="text-gray-600">' . $rotulos[$campo] . ':</span>';
    ...
    $camposExtras = array_diff(array_keys($data), array_merge(...array_values($grupos)));
...
<body class="min-h-screen">
    ...
    <div class="bg-indigo-600 text-white shadow-md">
        <div class="container mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <div>
                <a href="<?= $_SESSION['role'] === 'geraladm' ? 'mudar_km_carro.php' : 'admin.php' ?>"
        ...
        <div class="card p-6 mb-6 fade-in">
            ...
            <form method="get" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-3 flex justify-between">
                    ...
                    <a href="historico_corridas.php" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md transition">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 fade-in">
            <div class="card p-6 flex items-center">
                ...
                <div>
                    ...
                    <?php
                    $stmt_count_add = $conn->prepare("SELECT COUNT(*) FROM historico_alteracoes WHERE tipo_operacao = 'ADICAO'");
            ...
            <div class="card p-6 flex items-center">
                ...
                <div>
                    ...
                    <?php
                    $stmt_count_edit = $conn->prepare("SELECT COUNT(*) FROM historico_alteracoes WHERE tipo_operacao = 'ALTERACAO'");
            ...
            <div class="card p-6 flex items-center">
                ...
                <div>
                    ...
                    <?php
                    $stmt_count_del = $conn->prepare("SELECT COUNT(*) FROM historico_alteracoes WHERE tipo_operacao = 'EXCLUSAO'");
        ...
        <div class="card p-6 mb-6 fade-in">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Registro ID</th>
                ...
                <div id="cardView" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" style="display: none;">
                    <?php foreach ($historico as $item): ?>
                        <div class="history-card card p-4 border-l-4 <?= $item['tipo_operacao'] == 'ADICAO' ? 'border-green-500' : ($item['tipo_operacao'] == 'EXCLUSAO' ? 'border-red-500' : 'border-blue-500') ?>">
                            ...
                            <div class="mb-3">
                                <div class="text-xs font-medium text-gray-500 uppercase">Registro ID</div>
            ...
            <?php else: ?>
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500">Não foram encontrados registros com os filtros aplicados.</p>
                    <?php if (!empty($tipo_operacao) || !empty($data_inicial) || !empty($data_final) || !empty($registro_id) || !empty($admin_name) || !empty($campo_alterado)): ?>
                        <a href="historico_corridas.php" class="mt-4 inline-block bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md transition">
                            Limpar Filtros
                        </a>
                    <?php endif; ?>
                </div>
    ...
    $(document).ready(function() {
        ...
        $('#exportCSV').on('click', function() {
            ...
            let url = 'csv_corridas.php?';
            const params = new URLSearchParams(window.location.search);
            url += params.toString();
            
            // Redirecionar para o script de exportação
            window.location.href = url;"csv_corridas.php"
        });
    });
    </script>
</body>
