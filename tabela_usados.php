    ...
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="card p-5 mb-6 bg-white">
            <form id="filter-form" method="post" class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex-1">
                    ...
                    <div class="relative max-w-md">
                        ...
                        <input type="text" id="search" name="search" class="block w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300">
                    </div>
                </div>
            ...
            <div class="flex flex-wrap gap-2 mt-5">
                <button onclick="filterByConsumo('otimo')" class="filter-btn flex items-center px-3.5 py-1.5 rounded-full bg-green-50 text-green-800 hover:bg-green-100 text-sm font-medium border border-green-200">
                ...
                <button onclick="filterByConsumo('bom')" class="filter-btn flex items-center px-3.5 py-1.5 rounded-full bg-yellow-50 text-yellow-800 hover:bg-yellow-100 text-sm font-medium border border-yellow-200">
                ...
                <button onclick="filterByConsumo('ruim')" class="filter-btn flex items-center px-3.5 py-1.5 rounded-full bg-red-50 text-red-800 hover:bg-red-100 text-sm font-medium border border-red-200">
    ...
    <script>
        ...
        function updateVehicleDisplay(filteredVehicles = null) {
            ...
            container.innerHTML = vehiclesToDisplay.map(vehicle => {
                ...
                return `
                <div class="card vehicle-card p-5 hover:border-indigo-200" onclick="showVehicleDetails('${vehicle.prefixo || ''}')">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        ...
                        <div class="flex flex-col items-end">
                            ...
                            <button class="mt-2 text-xs text-indigo-600 hover:text-indigo-800 font-medium flex items-center">
        ...
        // Função para mostrar o modal com detalhes do veículo
        async function showVehicleDetails(prefixo) {
            // Abrir o modal e mostrar indicador de carregamento
            ...
            try {
                ...
                const response = await fetch(`get_vehicle_details.php?prefixo=${encodeURIComponent(prefixo)}`);
                ...
                const statusClass = data.status === 'em uso' ? 'status-em-uso' : 'status-livre';
                const statusText = data.status === 'em uso' ? 'Em uso' : 'Livre';
                
                // Verificar se o veículo está em uso para exibir informações adicionais
                if (data.status === 'em uso') {
                    modalContent.innerHTML = `
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-xl font-semibold">${data.prefixo || 'N/I'}</h3>
                            <span class="status-badge ${statusClass}">${statusText}</span>
                        </div>
                        <p class="text-gray-600 mb-6">${data.descricao || 'Veículo municipal'}</p>
                        
                        <div class="detail-row">
                            <div class="detail-label">Motorista:</div>
                            <div class="detail-value">${data.motorista || 'Não informado'}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">CPF:</div>
                            <div class="detail-value">${data.cpf || 'Não informado'}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Destino:</div>
                            <div class="detail-value">${data.destino || 'Não informado'}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Data de saída:</div>
                            <div class="detail-value">${data.data || 'Não informada'}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Hora de saída:</div>
                            <div class="detail-value">${data.hora_saida || 'Não informada'}</div>
                        </div>
                        <div class="detail-row">
                            <div class="detail-label">Secretaria:</div>
                            <div class="detail-value">${data.secretaria || 'Não informada'}</div>
                        </div>
                    `;
                } else {
                    // Veículo livre, exibe último uso
                    modalContent.innerHTML = `
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-xl font-semibold">${data.prefixo || 'N/I'}</h3>
                            <span class="status-badge ${statusClass}">${statusText}</span>
                        ...
                        <h4 class="text-lg font-medium text-gray-800 mb-4">Último uso registrado:</h4>
                        
                        ${data.ultimo_uso ? `
                            <div class="detail-row">
                                <div class="detail-label">Último motorista:</div>
                                <div class="detail-value">${data.ultimo_uso.motorista || 'Não informado'}</div>
                            </div>
                            <div class="detail-row">
                                <div class="detail-label">Último CPF:</div>
                                <div class="detail-value">${data.ultimo_uso.cpf || 'Não informado'}</div>
                            </div>
                            <div class="detail-row">
                                <div class="detail-label">Último destino:</div>
                                <div class="detail-value">${data.ultimo_uso.destino || 'Não informado'}</div>
                            </div>
                            <div class="detail-row">
                                <div class="detail-label">Data de retorno:</div>
                                <div class="detail-value">${data.ultimo_uso.data_retorno || 'Não informada'}</div>
                            </div>
                            <div class="detail-row">
                                <div class="detail-label">Hora de retorno:</div>
                                <div class="detail-value">${data.ultimo_uso.hora_retorno || 'Não informada'}</div>
                            </div>
                        ` : `
                            <div class="empty-state">
                                <i class="fas fa-info-circle"></i>
                                <p>Nenhum registro de uso anterior encontrado.</p>
                            </div>
                        `}
